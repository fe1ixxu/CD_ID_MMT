    ...
    def sizes(self):
        ...
        for ds, sr in zip(self.datasets, self.sample_ratios):
            if isinstance(ds.sizes, np.ndarray):
                max_dim = max(max_dim, ds.sizes.ndim)
        for ds, sr in zip(self.datasets, self.sample_ratios):
            if isinstance(ds.sizes, np.ndarray):
                new_sizes = ds.sizes
                if ds.sizes.ndim < max_dim:
                    assert ds.sizes.ndim == 1, "The smaller dataset dim can only be 1!"
                    new_sizes = np.vstack([ds.sizes]*max_dim).transpose()
                _dataset_sizes.append(np.tile(new_sizes, sr))
            else:
