    ...
    def __getitem__(self, index):
        if "net_input.src_tokens" in self.dataset[index]:
            if self.append_lgtoken:
                ...
                encoder_target = self.dataset[index]["target"].clone()
                encoder_target[-1] = self.dataset[index]["tgt_lang"]
            else:
                encoder_target = None
...
@register_task("translation_ssl_multitask2")
class TranslationSslMultitaskTask2(TranslationMultiSimpleEpochTask):
    @staticmethod
    def add_args(parser):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if "valid" not in split:
            ...
            if "mass" in self.args.ssl_tasks:
                self.load_mlm_dataset(split=split, epoch=epoch, combine=combine, task_name="mass", **kwargs)
            if "cd" in self.args.ssl_tasks:
                self.load_mlm_dataset(split=split, epoch=epoch, combine=combine, task_name="cd", **kwargs)
    ...
    def _get_task_loss(self, sample, net_output, encoder_out, net_output2, encoder_out2, model, criterion, task_name, device):
        ...
        encoder_target_key = "encoder_target" if self.args.append_lgtoken else "target"
        if task_name == "mlm":
            ...
            loss, nll_loss = criterion.compute_loss(
                ...
                (logits, None),
                {
                    ...
                    "target": sample[encoder_target_key][indices, :min_ind],
                },
                reduce=True,
            )  
    ...
    def _get_loss(self, sample, model, criterion):
        ...
        if ("id_mlm2" in sample["task_name"] or "id_mlm" in sample["task_name"] or "id_dae" in sample["task_name"] ) or (self.args.enable_id_mmt and "mmt" in sample["task_name"]):
            # second pass
    ...
    def train_step(
        ...
        # if "id_mlm2" in sample["task_name"]:
        ...
        #     print(sample["encoder_target"], sample["encoder_target"].shape)
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, constraints=None):
        ...
        # print(src_langtok_spec,tgt_langtok_spec)
        # print(self.args.source_lang, self.args.target_lang)
        # exit(0)
