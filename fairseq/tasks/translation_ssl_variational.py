    ...
    def __getitem__(self, index):
        if "net_input.src_tokens" in self.dataset[index]:
            ...
            return {
                ...
                "src_lang": self.dataset[index]["tgt_lang"],
            }
        else:
            ...
            if isinstance(self.dataset[index], tuple):
                return (
                    self.dataset[index][0],
                    {
                        ...
                        "src_lang": self.dataset[index][1]["src_lang"],
                ...
                return {
                    ...
                    "task_name": self.task_name,
                    "lang_id": self.dataset[index]["tgt_lang"],
                    "src_lang": self.dataset[index]["src_lang"],
                }

@register_task("translation_ssl_variational")
class TranslationSslVariational(TranslationMultiSimpleEpochTask):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument("--variational_training", action='store_true',
                            help='set true if using variational transformer architecture')
    ...
    def load_translation_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        for split, dataset in split_datasets.items():
            self.datasets[split] = SimpleSSLModelDataset(dataset, task_name="mmt")
    ...
    def _get_task_loss(self, sample, net_output, encoder_out, model, criterion, task_name, device):
        indices = torch.tensor([i for i, task in enumerate(sample["task_name"]) if task == task_name]).to(device)
        if len(indices) == 0:
            # dummy 0 loss
            return [torch.tensor([0]).to(device)]* 4 + [indices]
        ...
        if task_name == "mlm":
            ...
            ## Pass 2 decoder 
            logits_decoder2 = model(**sample["net_input"], step="decoder", encoder_out=encoder_out2)[0][indices, :min_ind, :]
        ...
        return loss, nll_loss, id_mlm_consis_loss, id_dae_consis_loss, indices
    ...
    def _get_loss(self, sample, model, criterion):
        ...
        variational_loss, lang_class_loss = torch.tensor([0]).to(device), torch.tensor([0]).to(device)

        if not self.args.variational_training:
            encoder_out = model(**sample["net_input"], step="encoder")
            net_output = model(**sample["net_input"], step="decoder", encoder_out=encoder_out)
        else:
            encoder_out = model(**sample["net_input"], step="encoder", src_langs=sample["src_langs"], tgt_langs=sample["tgt_langs"])
            variational_loss, lang_class_loss = encoder_out["variational_loss"][0], encoder_out["lang_class_loss"][0]
            net_output = model(**sample["net_input"], step="decoder", encoder_out=encoder_out)
            
        mlm_loss, mlm_nll_loss, _, _,  mlm_ind = self._get_task_loss(sample, net_output, encoder_out, model, criterion, "mlm", device)
        dae_loss, dae_nll_loss, _, _, dae_ind = self._get_task_loss(sample, net_output, encoder_out, model, criterion, "dae", device)
        id_mlm_loss, id_mlm_nll_loss, id_mlm_consis_loss, _, id_mlm_ind = self._get_task_loss(sample, net_output, encoder_out, model, criterion, "id_mlm", device)
        id_mlm_loss, id_mlm_nll_loss, id_mlm_consis_loss, _, id_mlm_ind = self._get_task_loss(sample, net_output, encoder_out, model, criterion, "id_mlm2", device)
        id_dae_loss, id_dae_nll_loss, _, id_dae_consis_loss, id_dae_ind = self._get_task_loss(sample, net_output, encoder_out, model, criterion, "id_dae", device)
        mmt_loss, mmt_nll_loss, _, _, mmt_ind = self._get_task_loss(sample, net_output, encoder_out, model, criterion, "mmt", device)
        ...
        loss = mmt_loss + dae_loss + mlm_loss + id_mlm_loss + id_dae_loss + variational_loss * sample_size + lang_class_loss * sample_size
        nll_loss = mmt_nll_loss + dae_nll_loss + mlm_nll_loss + id_mlm_nll_loss + id_dae_nll_loss
        ...
        logging_output = {
            ...
            "variational_loss": variational_loss.data,
            "lang_class_loss": lang_class_loss.data,
            "ntokens": sample["ntokens"],
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        variational_loss_sum = sum(log.get("variational_loss", 0) for log in logging_outputs)
        lang_class_loss_sum = sum(log.get("lang_class_loss", 0) for log in logging_outputs)
        ...
        metrics.log_scalar(
            "variational_loss", variational_loss_sum, mmt_sample_size, round=3
        )
        metrics.log_scalar(
            "lang_class_loss", lang_class_loss_sum, mmt_sample_size, round=3
        )
