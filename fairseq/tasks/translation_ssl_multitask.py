...
@register_task("translation_ssl_multitask")
class TranslationSslMultitaskTask(TranslationMultiSimpleEpochTask):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument("--ssl-resample", type=float, default=0.0,
                            help='The sample ratio between ssl and parallel data training, enabled when the number is strictly larger than 0.0')
    ...
    def setup_ssl_task(cls, args, dicts, **kwargs):
        ...
        return paths, dicts
    def prepare_ssl_dataset(self, dataset, epoch, task_name):
        with data_utils.numpy_seed(self.args.seed + epoch):
            shuffle = np.random.permutation(len(dataset))
        return SimpleSSLModelDataset(
            SortDataset(
                dataset,
                sort_order=[
                    shuffle,
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        assert not ("mlm" in self.args.ssl_tasks and "id_cd" in self.args.ssl_tasks), "Only one of mlm and id_cd is allowed!"
        ...
        if "valid" not in split:
            ssl_datasets = []
            ssl_data_total_length_before_tokenblock = 0
            if "mlm" in self.args.ssl_tasks:
                ssl_dataset, ssl_data_length_before_tokenblock = self.load_mlm_dataset(split=split, epoch=epoch, combine=combine, task_name="mlm", **kwargs)
                ssl_datasets.append(self.prepare_ssl_dataset(ssl_dataset, epoch, "mlm"))
                ssl_data_total_length_before_tokenblock += ssl_data_length_before_tokenblock
            if "id_mlm" in self.args.ssl_tasks:
                ssl_dataset, ssl_data_length_before_tokenblock = self.load_mlm_dataset(split=split, epoch=epoch, combine=combine, task_name="id_mlm", **kwargs)
                ssl_datasets.append(self.prepare_ssl_dataset(ssl_dataset, epoch, "id_mlm"))
                ssl_data_total_length_before_tokenblock += ssl_data_length_before_tokenblock
            if "id_cd" in self.args.ssl_tasks:
                ssl_dataset, ssl_data_length_before_tokenblock = self.load_mlm_dataset(split=split, epoch=epoch, combine=combine, task_name="id_cd", **kwargs)
                ssl_datasets.append(self.prepare_ssl_dataset(ssl_dataset, epoch, "id_cd"))
                ssl_data_total_length_before_tokenblock += ssl_data_length_before_tokenblock
            if "dae" in self.args.ssl_tasks:
                ssl_dataset, ssl_data_length_before_tokenblock = self.load_dae_dataset(split=split, epoch=epoch, combine=combine, task_name="dae", **kwargs)
                ssl_datasets.append(self.prepare_ssl_dataset(ssl_dataset, epoch, "dae"))
                ssl_data_total_length_before_tokenblock += ssl_data_length_before_tokenblock
            if "id_dae" in self.args.ssl_tasks:
                ssl_dataset, ssl_data_length_before_tokenblock = self.load_dae_dataset(split=split, epoch=epoch, combine=combine, task_name="id_dae", **kwargs)
                ssl_datasets.append(self.prepare_ssl_dataset(ssl_dataset, epoch, "id_dae"))
                ssl_data_total_length_before_tokenblock += ssl_data_length_before_tokenblock
            if "mass" in self.args.ssl_tasks:
                ssl_dataset, ssl_data_length_before_tokenblock = self.load_mlm_dataset(split=split, epoch=epoch, combine=combine, task_name="mass", **kwargs)
                ssl_datasets.append(self.prepare_ssl_dataset(ssl_dataset, epoch, "mass"))
                ssl_data_total_length_before_tokenblock += ssl_data_length_before_tokenblock
            if "cd" in self.args.ssl_tasks:
                ssl_dataset, ssl_data_length_before_tokenblock = self.load_mlm_dataset(split=split, epoch=epoch, combine=combine, task_name="cd", **kwargs)
                ssl_datasets.append(self.prepare_ssl_dataset(ssl_dataset, epoch, "cd"))
                ssl_data_total_length_before_tokenblock += ssl_data_length_before_tokenblock
            ssl_datasets = ConcatDataset(ssl_datasets)
            with data_utils.numpy_seed(self.args.seed + epoch):
                shuffle = np.random.permutation(len(ssl_datasets))
            if self.args.ssl_resample <= 0:
                self.datasets[split] = ConcatDataset(
                    [self.datasets[split],
                    ssl_datasets]
                )
            else:
                ratio_para = (1 - self.args.ssl_resample) * (len(self.datasets[split]) + ssl_data_total_length_before_tokenblock) / len(self.datasets[split])
                ratio_ssl = self.args.ssl_resample * (len(self.datasets[split]) + ssl_data_total_length_before_tokenblock) / ssl_data_total_length_before_tokenblock
                logger.info(f"Up/down sampling ratios by para data and ssl data are {ratio_para}, {ratio_ssl}")
                logger.info(f"Original size vs Current size for Parallel data is {len(self.datasets[split])} and {(1 - self.args.ssl_resample) * (len(self.datasets[split]) + ssl_data_total_length_before_tokenblock)}")
                logger.info(f"Original size vs Current size for SSL data is {ssl_data_total_length_before_tokenblock} and {self.args.ssl_resample * (len(self.datasets[split]) + ssl_data_total_length_before_tokenblock)}")
                self.datasets[split] = ResamplingDataset(
                    self.datasets[split],
                    size_ratio=ratio_para,
                    seed=self.args.seed,
                    epoch=epoch,
                    replace=ratio_para >= 1.0,
                    )
                ssl_datasets = ResamplingDataset(
                    ssl_datasets,
                    size_ratio=ratio_ssl,
                    seed=self.args.seed,
                    epoch=epoch,
                    replace=ratio_ssl>= 1.0,
                    )
                self.datasets[split] = ConcatDataset(
                    [self.datasets[split],
                    ssl_datasets,]
    ...
    def load_mlm_dataset(self, split, epoch=1, combine=False, task_name="mlm", **kwargs):
        ...
        for lang_id, language in enumerate(languages):
            ...
            # if self.args.ssl_max_sample < len(dataset):
            ...
            data_length_before_tokenblock.append(len(dataset))
            ...
            if task_name != "mass":
                ...
                src_dataset, tgt_dataset = MassTokensDataset.apply_mask(
                    dataset,
                    self.source_dictionary,
                    pad_idx=self.source_dictionary.pad(),
                    mask_idx=self.mask_idx,
        ...
        if split == self.args.train_subset:
            ...
            data_length_before_tokenblock = np.array(data_length_before_tokenblock, dtype=float)
            ssl_data_ratio_before_after_tokenblock = data_length_before_tokenblock / dataset_lengths
            ssl_data_total_length_before_tokenblock = sum([len(resampled_lang_datasets[i]) * ssl_data_ratio_before_after_tokenblock[i] for i in range(len(resampled_lang_datasets))])
            logger.info(f"ssl data length ratio before after token block is {ssl_data_ratio_before_after_tokenblock}, and original total resampled length is {ssl_data_total_length_before_tokenblock}")
        ...
        return dataset, ssl_data_total_length_before_tokenblock
    ...
    def _get_task_loss(self, sample, net_output, encoder_out, net_output2, encoder_out2, model, criterion, task_name, device):
        ...
        if task_name == "mlm":
            ...
            if self.args.id_alpha > 0:
                ...
                id_mlm_consis_loss = X_loss([logits_encoder, logits_encoder2], pad_mask)
                loss = loss_encoder + self.args.id_alpha * id_mlm_consis_loss
            else:
                loss = loss_encoder

            del logits_encoder, logits_encoder2
            nll_loss = nll_loss_encoder

        elif task_name == "id_cd":
            #### Pass 1 encoder
            min_ind = self._get_most_left_pad_ind(sample["net_input"]["src_tokens"][indices, :])
            if self.args.not_shared_proj:
                logits_encoder = model(**sample["net_input"], step="encoder_proj_layer", layer_input=encoder_out["encoder_out"][0].transpose(0,1)[indices, :min_ind, :])
            ...
            if self.args.id_alpha > 0:
                ...
                logits_decoder, logits_decoder2 = F.softmax(logits_decoder, dim=-1), F.softmax(logits_decoder2, dim=-1)
                pad_mask = sample["target"][indices, :min_ind].eq(criterion.padding_idx)
                id_mlm_consis_loss = X_loss([logits_decoder, logits_decoder2], pad_mask)
                loss = loss_decoder + self.args.id_alpha * id_mlm_consis_loss
            else:
                loss = loss_decoder
            nll_loss =  nll_loss_decoder
    ...
    def _get_loss(self, sample, model, criterion):
        ...
        if ("id_cd" in sample["task_name"] or "id_mlm" in sample["task_name"] or "id_dae" in sample["task_name"] ) or (self.args.enable_id_mmt and "mmt" in sample["task_name"]):
            # second pass
        ...
        mass_loss, mass_nll_loss, _, _, _, mass_ind = self._get_task_loss(sample, net_output, encoder_out, net_output2, encoder_out2, model, criterion, "mass", device)
        cd_loss, cd_nll_loss, _, _, _, cd_ind = self._get_task_loss(sample, net_output, encoder_out, net_output2, encoder_out2, model, criterion, "cd", device)
        ...
        id_mlm_loss, id_mlm_nll_loss, id_mlm_consis_loss, _, _, id_mlm_ind = self._get_task_loss(sample, net_output, encoder_out, net_output2, encoder_out2, model, criterion, "id_cd", device)
        ...
        loss = mmt_loss + dae_loss + mlm_loss + id_mlm_loss + id_dae_loss + mass_loss + cd_loss
        nll_loss = mmt_nll_loss + dae_nll_loss + mlm_nll_loss + id_mlm_nll_loss + id_dae_nll_loss + mass_nll_loss + cd_nll_loss
        sample_size = (
        ...
        mass_sample_size = (
            sample["target"][mass_ind,:].size(0) if criterion.sentence_avg else
            max(int(sample["ntokens"] * (len(mass_ind)/ batch_size)), 1)
        )
        ...
        logging_output = {
            ...
            "mass_sample_size": mass_sample_size,
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        mass_loss_sum = sum(log.get("mass_loss", 0) for log in logging_outputs)
        ...
        mass_sample_size = sum(log.get("mass_sample_size", 0) for log in logging_outputs)
        ...
        metrics.log_scalar(
            "mass_loss", mass_loss_sum / mass_sample_size / math.log(2), mass_sample_size, round=3
        )
        metrics.log_scalar(
