...
LANG_PAIRS='eng-fuv,eng-kmb,eng-lug,eng-nya,eng-swh,eng-umb,eng-wol,eng-zul'
src=eng
TGTS='fuv,kmb,lug,nya,swh,umb,wol,zul'
...
SHARED=${8:-share}
BLOCK=${9:-block}
SUBSET=${10:--1} # number of monolingual data to use, -1 for all
REMOVE=${11:-"noremove"} # remove language from ssl


export WANDB_NAME=${SLURM_JOB_ID}_resample_id_eng_xx_alpha${ALPHA}_${ID_MMT}_temp${TEMP}_${TASKS}_${TYPE}_mask${MASK_RATIO}_${SHARED}_${BLOCK}_sub${SUBSET}_${REMOVE}
...
mkdir -p ${SAVE_PATH}
RANDOM_PORT=135$(( $RANDOM % 50 + 1 ))
## Train  ## --mask is for dae, and --mask-prob is for mlm
 srun --job-name x_${ALPHA}_${MASK_RATIO} --output ${SAVE_PATH}/train.%j --error ${SAVE_PATH}/train.stderr.%j  \
 ...
 --ssl-tasks ${TASKS} --nossl-langs ${REMOVE} --ssl-max-sample ${SUBSET} --beta 0.5 --id-alpha ${ALPHA} ${SHARED} ${BLOCK} ${ID_MMT} --append-lgtoken \
 ...
 --optimizer adam --min-params-to-wrap 100000000 --use-local-shard-size  --ddp-backend fully_sharded  \
 --save-dir ${SAVE_PATH} --skip-invalid-size-inputs-valid-test --memory-efficient-fp16 --wandb-project M8_32K
...
for tgt in ${TGTS//,/ }; do
    ...
    python interactive.py ${DATA_BIN} --path $SAVE_PATH/checkpoint_best-shard0.pt \
        ...
        --task translation_ssl_multitask3  --append-lgtoken \
        --bpe "sentencepiece" \
